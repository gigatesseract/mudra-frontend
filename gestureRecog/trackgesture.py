...
def saveROIImg(img):
    ...
    cv2.imwrite(myNN.path1+name + ".png", img)
    time.sleep(0.04)
    myNN.convertToGrayImg(myNN.path1, myNN.path2)
    
    
    # plot = np.zeros((512, 512, 3), np.uint8)
    # roi = skinMask(img, x0, y0, width, height, 0, plot)
...
def Main():
    ...
    while True:
        ...
        if ans == 1:
            ...
            print("I hope i served you well!")
            return 0
...
# path = "./"
        ...
        cv2.putText(frame, 'b - Toggle Binary/SkinMask',
                    (fx, fy + fh), font, size, (0, 255, 0), 1, 1)
        cv2.putText(frame, 'x - Toggle Background Sub Mask',
                    (fx, fy + 2*fh), font, size, (0, 255, 0), 1, 1)
        cv2.putText(frame, 'g - Toggle Prediction Mode',
                    (fx, fy + 3*fh), font, size, (0, 255, 0), 1, 1)
        cv2.putText(frame, 'q - Toggle Quiet Mode',
                    (fx, fy + 4*fh), font, size, (0, 255, 0), 1, 1)
